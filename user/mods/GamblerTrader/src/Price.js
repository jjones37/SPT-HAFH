...
class Price {
    ...
    generateContainerPrices() {
        ...
        for (let i = 0; i < mysteryContainerNames.length; i++) {
            const name = this.MysteryContainer.getName(mysteryContainerNames[i]);
     ...
     */
    getItemPrice(parent, currentItem, amount) {
        const itemHelper = this.container.resolve("ItemHelper");
        const override = this.MysteryContainer.getOverride(parent, currentItem);
        let currentPrice = 0;
     ...
     */
    getContainerPresetPrices(name, parent, rarities, items, amount = 1) {
        const itemHelper = this.container.resolve("ItemHelper");
        const weapons = new Weapons_1.Weapons();
        let prices = [];
        let weaponPricesPerTier = [];
        let tierTotal = 0;
        let sum = 0;
        ...
        for (let i = 0; i < rarities.length; i++) {
            ...
            for (let j = 0; j < items.presets[i].length; j++) {
                for (let k = 0; k < items.presets[i][j].Items.length; k++) {
                    ...
                    if (name == 'helmet') {
                        if (helmet_slots.includes(items.presets[i][j].Items[k].slotId))
                            continue; // skip usless helmet attachments
                        //if (i == 3 && j == 0) { // Testing
     ...
     */
    generatePrices = (name, odds, prices, profitability) => {
        let sum = 0;
        let trackOdds = 0;
    ...
    loadoutSimulation = () => {
        ...
        for (let i = 0; i < simulations; i++) {
            const gamble = new Gamble_1.Gamble(this.container, this.config, this.logger, 'loadout');
            gamble.newGamble();
